...
class Invoice
{
    ...
    public function generar_xml($render = true)
    {
        ...
        if ($documento->facturacion->codigo_tipo_factura == '1001') {
            // Extraer tipo y porcentaje
            list($tipo, $porc) = explode('/', $documento->facturacion->tipo_detraccion);
            $documento->tipo_detraccion = $tipo;
            $documento->porcentaje_detraccion = floatval($porc);

            // 1. Cálculo bruto de la detracción
            $rawDet = $documento->total_venta * ($documento->porcentaje_detraccion / 100);

            // 2. Aplicar redondeo SUNAT (entero)
            $documento->detraccion = $this->aplicarRedondeoSUNAT($rawDet);

            // 3. Convertir a soles (si tienes tipo de cambio)
